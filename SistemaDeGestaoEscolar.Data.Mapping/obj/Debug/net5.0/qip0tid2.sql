...
CREATE TABLE [identity_user] (
    ...
    [senha_confirmacao] nvarchar(max) NULL,
...
VALUES (N'20230516204100_AddIdentity', N'5.0.5');
GO
...
WHERE ([d].[parent_object_id] = OBJECT_ID(N'[identity_user]') AND [c].[name] = N'senha_confirmacao');
IF @var0 IS NOT NULL EXEC(N'ALTER TABLE [identity_user] DROP CONSTRAINT [' + @var0 + '];');
ALTER TABLE [identity_user] ALTER COLUMN [senha_confirmacao] nvarchar(100) NOT NULL;
ALTER TABLE [identity_user] ADD DEFAULT N'' FOR [senha_confirmacao];
GO
...
FROM [sys].[default_constraints] [d]
INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
WHERE ([d].[parent_object_id] = OBJECT_ID(N'[identity_user]') AND [c].[name] = N'senha');
IF @var1 IS NOT NULL EXEC(N'ALTER TABLE [identity_user] DROP CONSTRAINT [' + @var1 + '];');
ALTER TABLE [identity_user] ALTER COLUMN [senha] nvarchar(100) NOT NULL;
ALTER TABLE [identity_user] ADD DEFAULT N'' FOR [senha];
GO

INSERT INTO [__EFMigrationsHistory] ([MigrationId], [ProductVersion])
VALUES (N'20230520121100_AddUserColumnSenha', N'5.0.5');
GO
